...
Complete guide to configuring Motion.
...
Motion looks for configuration in the following locations (in order):

1. `$XDG_CONFIG_HOME/motion/core.toml`
2. `~/.config/motion/core.toml` (fallback)
...
1. **Environment variables** (e.g., `MOTION_API_URL`)
...
**Location:** `$XDG_CONFIG_HOME/motion/server.toml` (fallback: `~/.config/motion/server.toml`)
...
```bash
export MOTION_API_KEY=your-secret-api-key-1
```
...
```toml
[storage]
database_url = "~/.local/share/motion/tasks.db"  # SQLite database location
...
**Default location:** `$XDG_DATA_HOME/motion/tasks.db` (fallback: `~/.local/share/motion/tasks.db`)

## Data Storage

### Database

**Location:** `$XDG_DATA_HOME/motion/tasks.db` (fallback: `~/.local/share/motion/tasks.db`)
...
```bash
cp ~/.local/share/motion/tasks.db ~/.local/share/motion/tasks.db.backup
```
...
**Location:** `$XDG_DATA_HOME/motion/notes/` (fallback: `~/.local/share/motion/notes/`)
...
| -------- | ---- | ------- | ----------- |
| `MOTION_REGION_COUNTRY` | string | `None` | ISO 3166-1 alpha-2 country code |
| `MOTION_STORAGE_BACKEND` | string | `"sqlite"` | Storage backend type |
| `MOTION_STORAGE_DATABASE_URL` | string | XDG path | Database file location |

**Example:**

```bash
# Production settings
export MOTION_REGION_COUNTRY=US
```
...
| Variable | Type | Default | Description |
| -------- | ---- | ------- | ----------- |
| `MOTION_API_HOST` | string | `"127.0.0.1"` | API server host |
| `MOTION_API_PORT` | int | `8000` | API server port |
| `MOTION_API_KEY` | string | `None` | API key for authentication |
| `MOTION_GANTT_MIN_DISPLAY_DAYS` | int | `56` | Minimum days in TUI Gantt chart |
...
# Config file will be: /custom/path/motion/core.toml
```
...
# Database will be: /custom/path/motion/tasks.db
# Notes will be: /custom/path/motion/notes/
```

### EDITOR

Set default text editor for `motion note` command:
...
# Server: motion-server (uses default 127.0.0.1:8000)
...
# Storage Settings
[storage]
database_url = "~/.local/share/motion/tasks.db"
backend = "sqlite"
```
...
Motion server supports API key authentication. Configure keys in `server.toml`:
...
```bash
motion --api-key "your-secret-key" table
```
...
```toml
[storage]
database_url = "~/Documents/motion/my-tasks.db"
backend = "sqlite"
```
...
1. Ensure `[ui]` section is present in `~/.config/motion/cli.toml`
2. Restart TUI: `motion tui`
...
4. Use `--force` flag to re-optimize: `motion optimize --force`
...
2. Ensure parent directory exists: `mkdir -p ~/.local/share/motion`
3. Check `database_url` path in config file
4. Verify permissions: `ls -la ~/.local/share/motion/`
...
6. **Test configuration changes** - Run `motion table` after config changes to verify

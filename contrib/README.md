...
This directory contains deployment configurations and infrastructure files for running Motion Server.
...
docker compose exec motion-server motion table
docker compose exec motion-server motion add "New task" -p 100

# Load demo data
docker compose exec motion-server python scripts/demo_data.py
```
...
See [launchd/motion-server.plist](launchd/motion-server.plist) for the property list.

---

## Systemd User Service Setup

Motion Server can run as a systemd user service for automatic startup and management.
...
1. Install the `motion-server` command globally
2. Copy the systemd service file to `~/.config/systemd/user/motion-server.service`
3. Enable the service for automatic startup
4. Reload the systemd daemon

### Important: CLI/TUI Requires Running Server

**The `motion` CLI and TUI commands require the server to be running.** Without a running server, all CLI/TUI commands will fail with connection errors.
...
```bash
systemctl --user start motion-server
systemctl --user enable motion-server  # Auto-start on login
```
...
```bash
export MOTION_API_URL=http://127.0.0.1:8000
```
...
- Check if server is running: `systemctl --user status motion-server`
- Start the server: `systemctl --user start motion-server`

**Error: "API mode is required"**

- Configure host and port in config file (see Method A above)
- Or set MOTION_API_URL environment variable (see Method B above)
...
# View recent logs
journalctl --user -u motion-server -n 50

# View logs since boot
journalctl --user -u motion-server -b
```
...
- **Data directory**: `~/.local/share/motion/`
...
nano ~/.config/systemd/user/motion-server.service
...
```ini
ExecStart=%h/.local/bin/motion-server --host 0.0.0.0 --port 9000
```

**Enable development mode with auto-reload:**

```ini
ExecStart=%h/.local/bin/motion-server --host 127.0.0.1 --port 8000 --reload
```
...
- Check permissions on the data directory:
  ...
  ```bash
  ls -ld ~/.local/share/motion/
  ```

- Ensure the database file is not corrupted:

  ```bash
  sqlite3 ~/.local/share/motion/tasks.db "PRAGMA integrity_check;"
  ```
...
4. Reload the systemd daemon
5. Uninstall the `motion-server` command
...
- `ReadWritePaths=%h/.local/share/motion`: Allows writes only to data directory

...
class TestTablePresenter(unittest.TestCase):
    ...
    def test_present_converts_dto_to_view_models(self):
        ...
        # Create test tasks
        task1 = self.repository.create(name="Task 1", priority=1, status=DomainTaskStatus.PENDING)
        task2 = self.repository.create(
            name="Task 2", priority=2, status=DomainTaskStatus.IN_PROGRESS
        )
        ...
        self.assertEqual(view_models[0].status, PresentationTaskStatus.PENDING)
        self.assertEqual(view_models[1].id, task2.id)
        self.assertEqual(view_models[1].name, "Task 2")
        self.assertEqual(view_models[1].status, PresentationTaskStatus.IN_PROGRESS)
    ...
    def test_convert_status_maps_domain_to_presentation(self):
        ...
        self.assertEqual(
            TablePresenter.convert_status(DomainTaskStatus.PENDING),
            PresentationTaskStatus.PENDING,
        )
        self.assertEqual(
            TablePresenter.convert_status(DomainTaskStatus.IN_PROGRESS),
            PresentationTaskStatus.IN_PROGRESS,
        )
        self.assertEqual(
            TablePresenter.convert_status(DomainTaskStatus.COMPLETED),
            PresentationTaskStatus.COMPLETED,
        )
        self.assertEqual(
            TablePresenter.convert_status(DomainTaskStatus.CANCELED),
            PresentationTaskStatus.CANCELED,
        )
    ...
    def test_present_sets_is_finished_for_completed_task(self):
        ...
        task = self.repository.create(
            name="Completed task", priority=1, status=DomainTaskStatus.COMPLETED
        )
    ...
    def test_present_sets_is_finished_for_canceled_task(self):
        ...
        task = self.repository.create(
            name="Canceled task", priority=1, status=DomainTaskStatus.CANCELED
        )

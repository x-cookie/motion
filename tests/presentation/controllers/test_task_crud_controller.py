...
class TestTaskCrudController(unittest.TestCase):
    ...
    def test_archive_task_returns_task_operation_output(self):
        ...
        # Create a task
        task = Task(name="Test Task", priority=1, status=TaskStatus.PENDING, is_archived=False)
        task.id = self.repository.generate_next_id()
        self.repository.save(task)
    ...
    def test_restore_task_returns_task_operation_output(self):
        ...
        task = Task(name="Test Task", priority=1, status=TaskStatus.PENDING, is_archived=True)
        task.id = self.repository.generate_next_id()
        self.repository.save(task)

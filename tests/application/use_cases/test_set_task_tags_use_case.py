...
class TestSetTaskTagsUseCase(unittest.TestCase):
    ...
    def test_execute_sets_tags_on_task(self):
        ...
        input_dto = SetTaskTagsRequest(task_id=self.task.id, tags=["work", "urgent"])
    ...
    def test_execute_replaces_existing_tags(self):
        ...
        # Set initial tags
        input_dto1 = SetTaskTagsRequest(task_id=self.task.id, tags=["work", "urgent"])
        self.use_case.execute(input_dto1)

        # Replace with new tags
        input_dto2 = SetTaskTagsRequest(task_id=self.task.id, tags=["personal", "low-priority"])
    ...
    def test_execute_clears_tags_with_empty_list(self):
        ...
        # Clear tags
        input_dto2 = SetTaskTagsRequest(task_id=self.task.id, tags=[])
        result = self.use_case.execute(input_dto2)
    ...
    def test_execute_with_nonexistent_task_raises_error(self):
        ...
        input_dto = SetTaskTagsRequest(task_id=9999, tags=["work"])
    ...
    def test_execute_with_duplicate_tags_raises_error(self):
        ...
        input_dto = SetTaskTagsRequest(task_id=self.task.id, tags=["work", "urgent", "work"])

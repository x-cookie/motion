...
class TestThisWeekFilter(unittest.TestCase):
    ...
    @patch("application.queries.filters.this_week_filter.datetime")
    def test_filter_includes_task_with_deadline_this_week(self, mock_datetime):
        ...
        mock_datetime.now.return_value = datetime.combine(self.today, datetime.min.time())
    ...
    @patch("application.queries.filters.this_week_filter.datetime")
    def test_filter_includes_task_with_planned_period_overlapping_this_week(self, mock_datetime):
    ...
    def test_filter_calculates_week_correctly(self, mock_datetime):
        ...
        for day_offset in range(7):  # Monday=0 to Sunday=6
            current_day = datetime(2025, 1, 13) + timedelta(days=day_offset)  # Week starting Jan 13

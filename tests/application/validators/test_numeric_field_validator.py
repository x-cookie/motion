...
class TestNumericFieldValidator(unittest.TestCase):
    ...
    def test_validate_positive_float_success(self):
        ...
        # estimated_duration
        self.estimated_duration_validator.validate(10.5, self.task, self.mock_repository)
    ...
    def test_validate_zero_raises_error(self):
        ...
        with self.assertRaises(TaskValidationError) as context:
            self.estimated_duration_validator.validate(0, self.task, self.mock_repository)
    ...
    def test_validate_zero_float_raises_error(self):
        ...
        # estimated_duration
        with self.assertRaises(TaskValidationError) as context:
            self.estimated_duration_validator.validate(0.0, self.task, self.mock_repository)
        self.assertIn("Estimated duration must be greater than 0", str(context.exception))
    ...
    def test_validate_negative_raises_error(self):
        ...
        # estimated_duration
        with self.assertRaises(TaskValidationError) as context:
            self.estimated_duration_validator.validate(-5, self.task, self.mock_repository)
        self.assertIn("Estimated duration must be greater than 0", str(context.exception))
    ...
    def test_validate_negative_float_raises_error(self):
        ...
        # estimated_duration
        with self.assertRaises(TaskValidationError) as context:
            self.estimated_duration_validator.validate(-5.5, self.task, self.mock_repository)
        self.assertIn("Estimated duration must be greater than 0", str(context.exception))
    ...
    def test_validate_none_value_success(self):
        ...
        self.estimated_duration_validator.validate(None, self.task, self.mock_repository)
    ...
    def test_validate_boolean_behavior(self):
        ...
        self.estimated_duration_validator.validate(True, self.task, self.mock_repository)

        # False should fail validation (treated as 0)
        with self.assertRaises(TaskValidationError) as context:
            self.estimated_duration_validator.validate(False, self.task, self.mock_repository)
    ...
    def test_validate_dict_raises_error(self):
        ...
        with self.assertRaises(TaskValidationError) as context:
            self.priority_validator.validate({"value": 10}, self.task, self.mock_repository)
        self.assertIn("Invalid type for priority", str(context.exception))
    ...
    def test_field_name_formatting_in_error_message(self):
        ...
        self.assertTrue(
            "Estimated duration" in error_message or "estimated_duration" in error_message
        )

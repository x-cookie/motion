...
class TestTaskIndexManager(unittest.TestCase):
    ...
    def test_init_without_tasks(self) -> None:
        """Test initialization without tasks."""
        manager = TaskIndexManager()

        self.assertIsNone(manager.get_by_id(1))
        self.assertFalse(manager.has_task(1))

    def test_init_with_tasks(self) -> None:
        """Test initialization with tasks builds indexes."""
        manager = TaskIndexManager(self.tasks)

        self.assertEqual(manager.get_by_id(1), self.task1)
        self.assertEqual(manager.get_by_id(2), self.task2)
        self.assertEqual(manager.get_by_id(3), self.task3)

    def test_build_indexes_creates_task_index(self) -> None:
    ...
    def test_build_indexes_creates_position_index(self) -> None:
        """Test build_indexes creates position index correctly."""
        manager = TaskIndexManager()
        manager.build_indexes(self.tasks)

        self.assertEqual(manager.get_position(1), 0)
        self.assertEqual(manager.get_position(2), 1)
        self.assertEqual(manager.get_position(3), 2)

    def test_build_indexes_skips_tasks_without_id(self) -> None:
        ...
        self.assertIsNone(manager.get_by_id(None))

    def test_get_by_id_returns_task_when_exists(self) -> None:
    ...
    def test_get_by_ids_returns_existing_tasks(self) -> None:
        ...
        self.assertEqual(result[1], self.task1)
        self.assertEqual(result[2], self.task2)
        self.assertEqual(result[3], self.task3)

    def test_get_by_ids_skips_non_existing_tasks(self) -> None:
    ...
    def test_get_by_ids_returns_empty_dict_when_no_matches(self) -> None:
    ...
    def test_get_by_ids_with_empty_list(self) -> None:
    ...
    def test_get_position_returns_position_when_exists(self) -> None:
    ...
    def test_get_position_returns_none_when_not_exists(self) -> None:
    ...
    def test_add_task_adds_to_both_indexes(self) -> None:
        ...
        self.assertEqual(manager.get_by_id(10), new_task)
        self.assertEqual(manager.get_position(10), 0)

    def test_add_task_raises_value_error_when_task_has_no_id(self) -> None:
        ...
        self.assertIn("Cannot index task without ID", str(cm.exception))

    def test_update_task_updates_task_in_index(self) -> None:
        ...
        result = manager.get_by_id(2)
        self.assertEqual(result.name, "Updated Task 2")
        self.assertEqual(result.priority, 5)

    def test_update_task_does_not_update_position(self) -> None:
    ...
    def test_update_task_raises_value_error_when_task_has_no_id(self) -> None:
        ...
        self.assertIn("Cannot update task without ID", str(cm.exception))

    def test_update_task_raises_value_error_when_task_not_in_index(self) -> None:
        ...
        self.assertIn("Task with ID 999 not found in index", str(cm.exception))

    def test_remove_task_removes_from_both_indexes(self) -> None:
        """Test remove_task removes from both indexes."""
        manager = TaskIndexManager(self.tasks)

        manager.remove_task(2)

        self.assertIsNone(manager.get_by_id(2))
        self.assertIsNone(manager.get_position(2))
        self.assertFalse(manager.has_task(2))

    def test_remove_task_with_non_existent_id(self) -> None:
        ...
        self.assertTrue(manager.has_task(1))
        self.assertTrue(manager.has_task(2))

    def test_has_task_returns_true_when_task_exists(self) -> None:
        """Test has_task returns True when task exists."""
        manager = TaskIndexManager(self.tasks)

        self.assertTrue(manager.has_task(1))
        self.assertTrue(manager.has_task(2))
        self.assertTrue(manager.has_task(3))

    def test_has_task_returns_false_when_task_does_not_exist(self) -> None:
        """Test has_task returns False when task doesn't exist."""
        manager = TaskIndexManager(self.tasks)

        self.assertFalse(manager.has_task(999))

    def test_rebuild_position_index_updates_positions(self) -> None:
        ...
        self.assertEqual(manager.get_position(3), 0)
        self.assertEqual(manager.get_position(1), 1)
        self.assertEqual(manager.get_position(2), 2)

    def test_rebuild_position_index_does_not_affect_task_index(self) -> None:
    ...
    def test_rebuild_position_index_with_fewer_tasks(self) -> None:
        ...
        self.assertEqual(manager.get_position(1), 0)
        self.assertEqual(manager.get_position(3), 1)
        # Task 2's position is removed but task_index still has it
        self.assertIsNone(manager.get_position(2))
        self.assertEqual(manager.get_by_id(2), self.task2)

    def test_rebuild_position_index_skips_tasks_without_id(self) -> None:
    ...
    def test_index_consistency_after_multiple_operations(self) -> None:
        ...
        self.assertEqual(manager.get_by_id(1).name, "Updated Task 1")
        self.assertEqual(manager.get_position(1), 0)
        ...
        self.assertEqual(manager.get_by_id(3), task3)
    ...
    def test_large_number_of_tasks(self) -> None:
        ...
        large_task_list = [Task(id=i, name=f"Task {i}", priority=1) for i in range(1000)]
        manager = TaskIndexManager(large_task_list)

        # Should handle lookup efficiently
        self.assertEqual(manager.get_by_id(500).name, "Task 500")
        self.assertEqual(manager.get_position(750), 750)  # Position matches index in list
        self.assertTrue(manager.has_task(999))

    def test_get_by_ids_with_duplicates(self) -> None:

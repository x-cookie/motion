...
class BaseBatchCommandTest(unittest.TestCase):
    ...
    action_name = (
        None  # Present tense for errors: "start", "complete", "pause", "cancel"
    )
    controller_method = None  # Optional: "start_task", "complete_task", etc. (for controller-based commands)
    ...
    def test_single_task_success(self):
        ...
        # Setup
        result_task = Task(
            id=1, name="Test Task", priority=5, status=TaskStatus.COMPLETED
        )

...
This module provides a WebSocket client that connects to the motion-server
...
class WebSocketClient:
    ...
    This client connects to the motion-server's WebSocket endpoint and
    ...
    async def connect(self) -> None:
        ...
        """
        if not WEBSOCKETS_AVAILABLE:
            logger.warning("Cannot connect: websockets library not available")
            return
    ...
    def _process_message(self, message_str: str | bytes) -> None:
        ...
        try:
            ...
            if message.get("type") == "connected":
                ...
                logger.info("Received client ID: %s", self.client_id)

            if self.on_message:
                self.on_message(message)
        except json.JSONDecodeError:
            logger.warning("Invalid JSON received: %r", message_str)

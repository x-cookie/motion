...
class Task:
    ...
    Attributes:
        name: Task name
        priority: Task priority (higher number = higher priority), or None if not set
    ...
    def __post_init__(self) -> None:
        ...
        - Priority (if provided) is a positive integer
    ...
    def _validate_priority(self) -> None:
        """Validate priority is a positive integer when set."""
        if self.priority is not None and self.priority <= MIN_PRIORITY_EXCLUSIVE:
    ...
    def get_unschedulable_reason(self, force_override: bool = False) -> str | None:
        ...
        if self.is_finished:
            return f"Task is already {self.status.value}"

        if self.status == TaskStatus.IN_PROGRESS:
            return "Task is already in progress"

        if not self.estimated_duration or self.estimated_duration <= 0:
            return "Task has no estimated duration"

        if self.is_fixed:
            return "Task is fixed (cannot be rescheduled)"

        if self.planned_start and not force_override:

...
class TestGetEnv:
    ...
    def test_type_conversion(self, env_value, type_, expected):
        ...
        with patch.dict(os.environ, {"motion_TEST_VAR": env_value}, clear=False):
    ...
    def test_invalid_int_returns_default(self):
        ...
        with patch.dict(os.environ, {"motion_TEST_VAR": "not_a_number"}, clear=False):
    ...
    def test_invalid_float_returns_default(self):
        ...
        with patch.dict(os.environ, {"motion_TEST_VAR": "invalid"}, clear=False):
    ...
    def test_invalid_value_logs_warning_by_default(self, caplog):
        ...
        with (
            caplog.at_level(
                logging.WARNING, logger="motion_core.shared.config_loader"
            ),
            patch.dict(os.environ, {"motion_TEST_VAR": "invalid"}, clear=False),
        ):
            ConfigLoader.get_env("TEST_VAR", 10, int)

        assert len(caplog.records) == 1
        assert "Invalid value for environment variable" in caplog.records[0].message
        assert "MOTION_TEST_VAR" in caplog.records[0].message
    ...
    def test_empty_string_returns_default(self):
        ...
        with patch.dict(os.environ, {"motion_TEST_VAR": ""}, clear=False):
...
class TestGetEnvList:
    ...
    def test_parses_comma_separated_values(self):
        ...
        with patch.dict(
            os.environ, {"motion_TEST_LIST": "one,two,three"}, clear=False
        ):
    ...
    def test_empty_string_returns_default(self):
        """Test that empty string env var returns default."""
        with patch.dict(os.environ, {"motion_TEST_LIST": ""}, clear=False):

...
class TestConfigManagerEnvVars:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("motion_REGION_COUNTRY", "US", "region", "country", "US"),
            ("motion_STORAGE_BACKEND", "postgres", "storage", "backend", "postgres"),
            (
                "motion_STORAGE_DATABASE_URL",
                "sqlite:///test.db",
                "storage",
                "database_url",
                "sqlite:///test.db",
    ...
    def test_no_env_vars_uses_defaults(self):
        ...
        # Ensure no motion_ env vars are set
        motion_vars = [k for k in os.environ if k.startswith("motion_")]

        with patch.dict(os.environ, dict.fromkeys(motion_vars, ""), clear=False):
            # Remove the vars we just blanked
            for k in motion_vars:

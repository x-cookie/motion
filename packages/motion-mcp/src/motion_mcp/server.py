...
from motion_mcp.config.mcp_config_manager import McpConfig, load_mcp_config
...
def create_mcp_server(config: McpConfig | None = None) -> FastMCP:
    ...
    client = MotionApiClient(base_url, api_key=config.api.api_key)

    # Create MCP server
    mcp = FastMCP(config.server.name)

    # Register all tools
    from motion_mcp.tools import (
        task_audit,
        task_crud,
        task_decomposition,
        task_lifecycle,

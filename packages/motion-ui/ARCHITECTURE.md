...
This document describes the state management architecture used in the Motion TUI (Terminal User Interface).

## Overview

The Motion TUI uses a **hybrid state management approach** that combines three complementary patterns:
...
**Location**: `src/motion/tui/state/tui_state.py`
...
app = self.app
assert isinstance(app, MotionTUI)
...
**Location**: `src/motion/tui/events.py`
...
- TUIState implementation: `src/motion/tui/state/tui_state.py`
- Event definitions: `src/motion/tui/events.py`
- Main app: `src/motion/tui/app.py`

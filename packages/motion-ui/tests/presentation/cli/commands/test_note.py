...
class TestNoteCommand:
    ...
    def test_stdin_input(self):
        ...
        with patch("motion.cli.commands.note._read_content_from_source") as mock_read:
    ...
    def test_editor_mode_when_no_input(self):
        ...
        with (
            patch("motion.cli.commands.note._read_content_from_source") as mock_read,
            patch(
                "motion.cli.commands.note._edit_with_editor_via_shared"
            ) as mock_edit,
        ):
    ...
    def test_file_read_oserror(self):
        ...
        from motion.cli.commands.note import _read_content_from_source

        # Create a mock file path that will fail to read
        with patch("motion.cli.commands.note.Path") as mock_path_cls:
            mock_path = MagicMock()
    ...
    def test_empty_stdin_falls_back_to_editor(self):
        ...
        with (
            patch("motion.cli.commands.note.sys.stdin") as mock_stdin,
            patch("motion.cli.commands.note.select.select") as mock_select,
        ):
    ...
    def test_edit_with_editor_via_shared_calls_edit_task_note(self):
        ...
        from motion.cli.commands.note import _edit_with_editor_via_shared

        with patch("motion.cli.commands.note.edit_task_note") as mock_edit:

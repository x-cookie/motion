...
from motion.cli.commands.common_options import filter_options, sort_options
...
from motion.renderers.rich_gantt_renderer import RichGanttRenderer
...
\b
EXAMPLE:
  motion gantt                                  # Show non-archived tasks
  motion gantt -a                              # Show all tasks (including archived)
  motion gantt --status completed              # Show only completed tasks
  motion gantt -t work -t urgent               # Tasks with tag "work" OR "urgent"
  motion gantt --start-date 2025-10-01 --end-date 2025-10-31  # October tasks
""",
)
@click.option(
    "--tag",

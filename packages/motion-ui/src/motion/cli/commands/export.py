...
from motion.cli.commands.common_options import date_range_options, filter_options

if TYPE_CHECKING:
    from datetime import datetime

    from motion.cli.context import CliContext
from motion.exporters import (
    CsvTaskExporter,
    JsonTaskExporter,
...
def export_command(
    ...
    Examples:
        motion export                              # Export non-archived tasks as JSON
        motion export -a                           # Export all tasks (including archived)
        motion export --status completed           # Export only completed tasks
        motion export -t work -t urgent            # Export tasks with tag "work" OR "urgent"
        motion export -o tasks.json                # Save JSON to file
        motion export --format csv -o tasks.csv    # Export to CSV
        motion export --format markdown -o tasks.md  # Export to Markdown table
        motion export --fields id,name,priority    # Export only specific fields
        motion export -f id,name,status --format markdown  # Markdown with specific fields
        motion export -a --status archived -o archived.json      # Export all archived tasks
        motion export --start-date 2025-10-01 --end-date 2025-10-31  # October tasks
    """

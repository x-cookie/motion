...
def rm_command(ctx: click.Context, task_ids: tuple[int, ...], hard: bool) -> None:
    ...
    By default, tasks are archived (is_archived flag set to True) and can be restored with 'motion restore'.
    ...
    if hard:
        ...
        for result in results.results:
            if result.success and result.task is not None:
                ...
                console_writer.info(
                    f"Use 'motion restore {result.task_id}' to restore this task."
                )
            elif result.error is not None:
                console_writer.validation_error(result.error)
            if len(task_ids) > 1:

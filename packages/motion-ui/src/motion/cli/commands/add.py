...
def add_command(
    ...
    Usage:
        # Basic task
        motion add "Task name"

        # With priority and tags
        motion add "Task name" --priority 3 --tag backend

        # Full task with all properties
        motion add "Implement auth" -p 5 -D "2025-12-01" -e 8 -s "2025-11-20" -t backend

    Examples:
        # Simple task
        motion add "Implement authentication"

        # With priority
        motion add "Fix login bug" -p 5

        # With deadline and estimate
        motion add "Add unit tests" --deadline "2025-12-15" --estimate 4

        # Complete task with schedule
        motion add "Code review" -p 3 -s "2025-11-20 09:00" -e "2025-11-20 12:00"

        # With dependencies and tags
        motion add "Deploy feature" -d 123 -t deployment -p 5
    """
